...
class ParseReutersCollatedStories(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        attrs = dict(attrs)
        try:
            if tag == 'div' and attrs['class'].replace("\\'", "").lower() == 'headlineMed'.lower() :
                self.recording = True
...
class ParseReutersStory(HTMLParser):
    ...
    def handle_starttag(self, tag, attrs):
        ...
        try:
            if tag == 'meta' and attributes['name'] == "parsely-page" :
                to_evaluate = attributes["content"]
                properties = eval(to_evaluate)
                self.data['date'] = properties["pub_date"]
                self.data['headline'] = properties["title"]
                self.data['author'] = properties["author"] if properties["author"].strip() != "" else "None"
            if tag == 'span' and attributes['id'] == 'articleText':
...
if __name__ == '__main__':
    ...
    root = Element("news")
    archive_base_url = "http://www.reuters.com/resources/archive/us/"
    current_year = 2007
    current_month = 3
    ...
    while (not(current_year == 2014  and current_month == 10)):
        ...
        for day in year_calender_iter:
            if day == 0 :
                continue
            day_base_url = archive_base_url + str(current_year) + str(current_month).zfill(2) + str(day).zfill(2) + ".html"
            print(day_base_url)

            try:
                request = urllib.request.Request(day_base_url)
                request.add_header('User-Agent', 'Mozilla/5.0 (Windows NT 6.1; WOW64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/37.0.2062.120 Safari/537.36')
            ...
            for url, headline in collatedStoriesParser.data.items():
                for company_keyword in companies_keywords:
                    if str(headline).lower().find(company_keyword.lower()) != -1 and str(headline).lower().find(
                            'television') == -1 and str(headline).lower().find('review') == -1 and str(
                            headline).lower().find('music') == -1 and str(headline).lower().find(
                            'entertainment') == -1 and str(headline).lower().find('video') == -1 and str(headline).lower().find('film') == -1:
                        break
                else:
                    continue
                try:
        ...
        ElementTree.ElementTree(root).write("reuters-news-" + str(current_year) + "-" + str(current_month) + ".xml", xml_declaration=True)
        root = Element("news")
